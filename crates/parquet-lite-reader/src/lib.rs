use parquet2::{
  encoding::{
    bitpacked, delta_bitpacked, delta_length_byte_array,
    hybrid_rle::{Decoder as HybridRleDecoder, HybridEncoded},
    Encoding,
  },
  page::{split_buffer, DataPage, DictPage},
  read::{decompress, get_page_iterator, levels::get_bit_width, read_metadata},
...
fn decode_levels(
  ...
  for run in decoder {
    let run =
      run.map_err(|e| JsError::new(&format!("Failed to decode {} levels: {:?}", level_name, e)))?;
    match run {
...
fn convert_json_string_to_object(value: &JsValue) -> Result<JsValue, JsError> {
  if let Some(json_str) = value.as_string() {
    let parsed =
      js_sys::JSON::parse(&json_str).map_err(|_| JsError::new("Failed to parse JSON string"))?;
    return Ok(parsed);
  }
  Ok(value.clone())
}
...
fn slice_to_array<const N: usize>(slice: &[u8]) -> Result<[u8; N], JsError> {
  slice.try_into().map_err(|_| {
    JsError::new(&format!(
      "Invalid slice length for array conversion (expected {}, got {})",
      N,
      slice.len()
    ))
  })
  ...
  F: FnMut(&[u8]) -> Result<T, JsError>,
{
  ...
  for _ in 0..count {
    if offset + width > buffer.len() {
      return Err(JsError::new(
        "Unexpected end of buffer while decoding values",
      ));
    }
...
fn decode_boolean_values(buffer: &[u8], count: usize) -> Result<Vec<bool>, JsError> {
  ...
  for i in 0..count {
    ...
    if byte_idx >= buffer.len() {
      return Err(JsError::new(
        "Unexpected end of buffer while decoding booleans",
      ));
    }
...
fn decode_fixed_len_byte_array(
  ...
  for i in 0..count {
    ...
    if end > buffer.len() {
      return Err(JsError::new(
        "Unexpected end of buffer while decoding fixed-len byte array",
      ));
    }
...
fn decode_dictionary_indices(
  ...
  for run in decoder {
    let run =
      run.map_err(|e| JsError::new(&format!("Failed to decode dictionary indices: {:?}", e)))?;
    match run {
      HybridEncoded::Bitpacked(values) => {
...
pub fn read_parquet(data: &[u8], columns: Option<Vec<String>>) -> Result<JsValue, JsError> {
  ...
  for row_group in &metadata.row_groups {
    for (col_idx, col_descriptor) in leaf_columns.iter().enumerate() {
      ...
      if !columns_to_read.iter().any(|c| {
        c == &col_name
          || col_descriptor.path_in_schema.first().map(|s| s.as_str()) == Some(c.as_str())
      }) {
        continue;
      }
      ...
      for page_result in pages {
        let page =
          page_result.map_err(|e| JsError::new(&format!("Failed to read page: {:?}", e)))?;
        ...
        match page {
          ...
          parquet2::page::Page::Data(data_page) => {
            ...
            if max_rep_level > 0 {
              if let Some(rep_levels) =
                decode_repetition_levels(rep_levels_buf, max_rep_level, data_page.num_values())?
              {
            ...
            if is_dict_encoded {
              ...
              if let Some(ref dict) = dictionary {
                ...
                let values: Vec<JsValue> = indices
                  .iter()
                  .map(|&idx| dict.get(idx).cloned().unwrap_or(JsValue::NULL))
                  .collect();
              ...
              match physical_type {
                PhysicalType::Int32 | PhysicalType::Int64 => {
                  ...
                  let use_bigint = matches!(physical_type, PhysicalType::Int64)
                    && !matches!(
                      logical_type,
                      Some(
                        PrimitiveLogicalType::Timestamp { .. } | PrimitiveLogicalType::Time { .. }
                      )
                    );
              ...
              match physical_type {
                PhysicalType::Int32 => {
                  let values =
                    decode_fixed_width_values(values_buffer, present_values, 4, |chunk| {
                      let arr = slice_to_array::<4>(chunk)?;
                      Ok(i32::from_le_bytes(arr))
                    })?;
                ...
                PhysicalType::Int64 => {
                  let values =
                    decode_fixed_width_values(values_buffer, present_values, 8, |chunk| {
                      let arr = slice_to_array::<8>(chunk)?;
                      Ok(i64::from_le_bytes(arr))
                    })?;
                  let use_bigint = !matches!(
      ...
      let existing =
        js_sys::Reflect::get(&result, &JsValue::from_str(&col_name)).unwrap_or(JsValue::UNDEFINED);
